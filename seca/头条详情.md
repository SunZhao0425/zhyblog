...
ALTER TABLE `bo8tv`.`b8_personal_center_ad` 
MODIFY COLUMN `type` int(4) UNSIGNED NOT NULL DEFAULT 0 COMMENT '点击跳转类型 1-帖子 2-文章 3-视频 4-活动h5  5直播间' AFTER `is_show`,
ADD COLUMN `content_id` int(11) ZEROFILL DEFAULT 0 COMMENT '内容id/或房间号(类型为5用)' AFTER `url`;
        ...
        <div class="form-group has-success url" style="display: none">
            <label class="control-label" >跳转地址:</label>
            <input type="text" name="PersonalCenterAd[url]"        class ="form-control  "   placeholder="请输入跳转地址">
        </div>
        <div class="form-group has-success room" style="display: block">
            <label class="control-label" >房间号:</label>
            <input type="text" name="PersonalCenterAd[content_id]" class ="form-control  room"  placeholder="请输入直播间房间号" value="<?=(int)$model->content_id ?>">
        </div>
        <div class="form-group has-success content" style="display: none">
            <label class="control-label" >内容id:</label>
            <input type="text" name="PersonalCenterAd[content_id]" class ="form-control content" placeholder="请输入文章/帖子/视频的id">
        </div>